...
def bidiagonal_singular_values_dlasq1(
    ...
    builds.  Standard LAPACK-enabled installations commonly provide the required symbol.
...
def tntool_bd_matrix(params: FloquetParameters) -> np.ndarray:
    ...
    ``(n-r+i, i)`` stores ``l_i^(r)``.  TNTool's upper-triangular convention
    reverses the within-sweep order: entry ``(i, n-r+i)`` stores
    ``u_{r+1-i}^(r)``.  Indices in this docstring are one-based.  This ordering
    is the one used by TNTool's ``TNExpand`` routine and is required for the
    packed matrix to reconstruct the original ordered factor product.
    ...
    for r in range(1, n):
        ...
        for i in range(1, r + 1):
            ...
            bd[i - 1, row - 1] = params.upper[r][r + 1 - i]
    return bd


def tntool_expand_bd(bd: np.ndarray) -> np.ndarray:
    """Expand a TNTool ``BD(A)`` matrix using Koev's ``TNExpand`` convention.

    This is an independent Python translation of the two update loops in
    TNTool's ``TNExpand.m``.  It is used as a regression check on the packing
    convention and is not part of the Cauchon eigensolver.
    """
    bd = np.asarray(bd, dtype=float)
    if bd.ndim != 2 or bd.shape[0] != bd.shape[1]:
        raise ValueError("bd must be a square matrix")
    n = bd.shape[0]
    matrix = np.zeros_like(bd)
    np.fill_diagonal(matrix, np.diag(bd))

    # Lower bidiagonal factors, following TNExpand.m exactly.
    for offset in range(1, n):
        for j_1based in range(n, offset, -1):
            j = j_1based - 1
            matrix[j, :] += bd[j, j_1based - offset - 1] * matrix[j - 1, :]

    # Upper bidiagonal factors.
    for offset in range(1, n):
        for j_1based in range(n, offset, -1):
            j = j_1based - 1
            matrix[:, j] += bd[j_1based - offset - 1, j] * matrix[:, j - 1]
    ...
    *,
    ...
    for r in range(1, n):
        ...
        for i in range(1, r + 1):
            ...
            upper_r[r + 1 - i] = float(bd[i - 1, row - 1])
        lower.append(lower_r)
        upper.append(upper_r)
    return FloquetParameters(
        lower=lower,
